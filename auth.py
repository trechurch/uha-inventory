...
def _try_os_user() -> Optional[Dict]:
    ...
    # Default to editor for OS users; admins must be promoted in DB
    return {
...
def _render_login_form(db) -> Optional[Dict]:
    ...
    submit = st.button("Sign In", type="primary", key="_login_submit", use_container_width=False)
...
def why_prompt(
    ...
    """
    ph = placeholder or (
        "Required — describe why this change is being made." if required else "Optional — describe why this change is being made."
    )
        ...
        with st.expander(
            ...
            with col1:
                new_role = st.selectbox(
                    "Role",
                    ROLES,
                    index=list(ROLES).index(u.get("role", "editor")) if u.get("role", "editor") in ROLES else 1,
                    key=f"role_{uname}",
                )
            ...
            # PIN reset
            with col2:
                new_pin = st.text_input("New PIN", type="password", placeholder="leave blank to keep current", key=f"pin_{uname}")
                if st.button("Reset PIN", key=f"pin_btn_{uname}"):
                    if new_pin and len(new_pin) >= 4:
                        db.upsert_user({"username": uname, "pin_hash": _hash_pin(new_pin.strip(), uname)})
            ...
            st.caption(f"Email: {u.get('email') or '—'} · Method: {u.get('auth_method') or '—'} · Last login: {str(last)[:19]}")
